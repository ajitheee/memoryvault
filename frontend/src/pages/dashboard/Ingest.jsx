      ...
      <div className="p-8 grid lg:grid-cols-2 gap-8">
        <div className="border border-[#262626] p-6">
            ...
            {["user", "assistant", "system"].map((r) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-mono uppercase tracking-wider border transition-colors ${
                  role === r ? "bg-white text-black border-white" : "border-[#262626] text-neutral-400 hover:text-white"
                }`}
              >
                {r}
              </button>
          ...
          <textarea
            ...
            className="w-full bg-[#050505] border border-[#262626] p-4 text-sm outline-none focus:border-white transition-colors resize-none font-mono"
          />
          <button
            data-testid="ingest-submit"
            onClick={() => save.mutate()}
            disabled={!text.trim() || save.isPending}
            className="mt-4 w-full inline-flex items-center justify-center gap-2 bg-white text-black font-semibold py-2.5 hover:bg-neutral-200 transition-colors disabled:opacity-50"
          >
          ...
          <div className="mt-5 space-y-2">
            ...
            {SAMPLES.map((s, i) => (
              <button
                ...
                onClick={() => setText(s)}
                className="block w-full text-left text-xs text-neutral-400 border border-[#262626] p-2 hover:border-white/40 hover:text-white transition-colors"
              >
                {s}
              </button>
            ))}
          ...
          {!result ? (
            ...
            <div className="space-y-4" data-testid="ingest-result">
              ...
              <div className="space-y-px bg-[#262626] border border-[#262626]">
                {result.facts.map((f) => (
                  <div key={f.id} className="bg-[#0A0A0A] p-3 flex items-center gap-3">
                ...
                {result.facts.length === 0 && (
                  <div className="bg-[#0A0A0A] p-4 text-sm text-neutral-600">No durable facts found in this message.</div>
                )}
              </div>
            </div>
          )}
